...
public class BackgroundIndexer implements AutoCloseable {
     ...
     */
    public BackgroundIndexer(String index, SQLTransportExecutor client, int numOfDocs) {
     ...
     */
    public BackgroundIndexer(String index, SQLTransportExecutor client, int numOfDocs, final int writerCount) {
     ...
     * been indexed.
     *
     * @param table       table name to write into
     * @param numOfDocs   number of document to index before pausing. Set to -1 to have no limit.
     * @param writerCount number of indexing threads to use
                    ...
                    try {
                        ...
                        while (!stop.get()) {
                            if (batch) {
                                ...
                                insert.append(String.format("insert into %s (x) values ", table));
                                for (int i = 0; i < batchSize; i++) {
                                    id = idGenerator.incrementAndGet();
                                    insert.append(generateValues(threadRandom));
                                }
                                try {
                                    SQLResponse response = sqlExecutor.execute(insert.toString(), null).actionGet();
                                } catch (Exception e) {
                                ...
                                if (useAutoGeneratedIDs) {
                                    try {
                                        StringBuilder insert = new StringBuilder();
                                        insert.append(String.format("insert into %s (x) values(?) ", table));
                                        SQLResponse response = sqlExecutor.execute(insert.toString(), new Object[]{ generateValues(threadRandom) }).actionGet();
                                    } catch (Exception e) {
                                        if (ignoreIndexingFailures == false) {
                                            throw e;
                                        }
