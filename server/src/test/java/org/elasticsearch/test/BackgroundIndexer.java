...
public class BackgroundIndexer implements AutoCloseable {
     ...
     * been indexed.
     *
     * @param index     table name to write data
                    ...
                    try {
                        ...
                        while (!stop.get()) {
                            if (batch) {
                                ...
                                var insert = String.format("insert into %s (x) (select * from unnest(?))", table);
                                var insertData = new Object[batchSize];
                                for (int i = 0; i < batchSize; i++) {
                                    insertData[i] = new Object[] { generateValues(threadRandom) };
                                }
                                try {
                                    sqlExecutor.execute(insert, insertData).actionGet();
                                } catch (Exception e) {
                                ...
                                if (useAutoGeneratedIDs) {
                                    try {
                                        ...
                                        sqlExecutor.execute(insert.toString(),
                                                            new Object[]{generateValues(threadRandom)}).actionGet();
